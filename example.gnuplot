# example.gnuplut : configuration for plotting (change as needed)

reset                                   # reset
set size ratio 0.2                      # set relative size of plots
set grid xtics ytics                    # grid: enable both x and y lines
set grid lt 1 lc rgb '#cccccc' lw 1     # grid: thin gray lines
set multiplot layout 3,1 scale 1.0,1.0  # set two plots for this figure

# freq domain (PSD)
set title 'Demodulated FM'
set ylabel 'Spectrum (dB/Hz)'            # set y-axis label
set xlabel 'Frequency (KHz)'             # set x-axis label
set yrange [-60:0]                       # set y plot range
set xrange [0:120]                       # set x plot range
# add your own .dat file for PSD as part of the take-home
plot '../data/psd_est_demod.dat' using 1:2 with lines lt 1 lw 3 lc rgb '#880000'

set title 'Extracted Mono'
set ylabel 'PSD (dB/Hz)'               # set y-axis label
set xlabel 'Freq (kHz)'                   # set x-axis label
set yrange [-200:0]                       # set y plot range
set xrange [0:120]                      # set x plot range
plot '../data/psd_est_filt.dat' using 1:2 with lines lt 1 lw 2 lc rgb '#000088'

set title 'Mono Audio'
set ylabel 'PSD (dB/Hz)'              # set y-axis label
set xlabel 'Freq (kHz)'               # set x-axis label
set yrange [-140:0]                    # set y plot range
set xrange [0:25]                       # set x plot range
plot '../data/psd_est_block.dat' using 1:2 with lines lt 1 lw 2 lc rgb '#008800'


unset multiplot
