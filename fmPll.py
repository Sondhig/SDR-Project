...
def fmPll(pllIn, freq, Fs, \
        ncoScale ,state_save, phaseAdjust = 0.0, normBandwidth = 0.01):
    ...
    integrator = state_save[0]#0.0
    phaseEst = state_save[1]#0.0
    feedbackI = state_save[2]#1.0
    feedbackQ = state_save[3]#0.0
    ncoOut[0] = state_save[4]#1.0
    trigOffset = state_save[5]#0
    ...
    #ON LAST ITERATION SAVE STATES
    state_save[0]=integrator
    state_save[1]=phaseEst
    state_save[2]=feedbackI
    state_save[3]=feedbackQ
    state_save[4]=ncoOut[-1]
    state_save[5]=trigOffset+len(pllIn)
    # for stereo only the in-phase NCO component should be returned
    # for block processing you should also return the state
    return ncoOut,state_save
