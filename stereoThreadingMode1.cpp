...
#define DEMOD_SIZE 128000 // size of one block after demodulation
...
float expand_factor = 24.0;
float audio_Fs = 48e3;
float audio_decim = 125;
float audio_taps = 101 * expand_factor;
...
std::vector<float> i_filt, q_filt, pilot_tone_filt, stereo_filt, mixer, pll;
...
// rf thread. input: 2.5MHz 8bit unsigned int; output: 240kHz 32bit signed float
int rfThread(std::vector<float> rf_coeff, int block_size,
    ...
    demod_queue.push(fm_demod); // This is the new method
...
int audioThread(std::mutex &qtex, std::condition_variable &cvar) {
    ...
    // Upsampling by exand_factor
    std::vector<float> upsampled_demod(fm_demod.size * expand_factor,
                                       static_cast<float>(0));
    std::vector<float> upsampled_mixer(mixer.size() * expand_factor,
                                       static_cast<float>(0));

    Upsample(fm_demod, upsampled_demod, expand_factor);
    Upsample(mixer, upsampled_mixer, expand_factor);

    // stereo: convolution Fc=16k with decimation by 5, just like mono
    std::vector<float> mixer_block(upsampled_mixer.size() / audio_decim),
        audio_block(upsampled_demod.size() / audio_decim);
    convolveFIR(mixer_block, upsampled_mixer, audio_coeff, mixer_zi,
                audio_decim, expand_factor);
...
int main() {
    ...
    impulseResponseLPF(mode1_Fs, audio_Fc, audio_taps, audio_coeff);
    // stereo coefficients for BPF convolution
    impulseResponseStereo(pll_bpf_fb, pll_bpf_fe, mode1_Fs, audio_taps,
                          pilot_tone_coeff);
    impulseResponseStereo(stereo_bpf_fb, stereo_bpf_fe, mode1_Fs, audio_taps,
                          stereo_coeff);
